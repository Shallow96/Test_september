...
(function () {
    ...
    this.flipchartCss = '<link href="//megatimer.ru/timer/timer.min.css?v=3" rel="stylesheet" type="text/css">', this.MegaTimer = function () {
        ...
        return s = {
            ...
            var n, f, o, y, l, p, b, v, x, E, w, T, k, I, C, M, N, L, H, S;
            if (r && this.drawArc(m, d, h, u, t, e, c, "transparent", r, s, g), g || (1 === e ? t = 1 - e : e = 1 - e), E = this.hexToRgb(i), x = this.hexToRgb(a), T = this.hexToRgb(this.getColorBetween(E, x, t)), I = this.hexToRgb(this.getColorBetween(E, x, e)), b = Math.ceil(30 * Math.abs(e - t)), y = this.getGradientColors(T, I, b), n = -.5 * Math.PI + 2 * Math.PI * t, l = y.length, f = g ? (k = 2 * Math.PI * (e - t) / l, 1) : (k = -2 * Math.PI * (e + t) / l, -1), w = function (t, e, i) {
                var a, r, s, n, o, l, p;
                ...
                if (o = n + k * (l - 1), 1 < l) {
                    for (p = M = 0, L = (v = Math.ceil(l / 2)) - 1; 0 <= L ? M <= L : L <= M; p = 0 <= L ? ++M : --M) 0 === p && (C = k, k *= .01, w(p, n, s), k = C), w(p, n, "butt"), n += k;
                    for (n = o, S = [], p = N = H = l - 1; H <= v ? N <= v : v <= N; p = H <= v ? ++N : --N) p === l - 1 && (C = k, k *= .01, w(p, n + .99 * C, s), k = C), w(p, n, "butt"), S.push(n -= k);
                    return S
                }
